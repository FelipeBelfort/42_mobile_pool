...
class TabViewManager {
  ...
  static void setData(Map<String, dynamic> cityData) {
    ...
    TabViewManager.curr = {};
    TabViewManager.daily = {};
    TabViewManager.hourly = {};
    updateWeather();
  }
    ...
    return Expanded(
      child: LineChart(
        LineChartData(
          ...
          lineBarsData: [
            LineChartBarData(
              spots: List.generate(24, (index) => FlSpot(index.toDouble(), hourly['temperature_2m'][index])),
              isCurved: false,
              color: Colors.orange,
              barWidth: 1.5,
    ...
    return Expanded(
      child: LineChart(
        LineChartData(
              ...
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (value, meta) {
                  ...
                  if (index >= 0 && index < daily['time'].length) {
                    return Text('${daily['time'][index].split('-')[2]}');
                  }
                  return Container();
                },
                reservedSize: 20,
          ...
          lineBarsData: [
            LineChartBarData(
              spots: List.generate(daily['time'].length, (index) => FlSpot(index.toDouble(), daily['temperature_2m_max'][index])),
              isCurved: false,
            ...
            LineChartBarData(
              spots: List.generate(daily['time'].length, (index) => FlSpot(index.toDouble(), daily['temperature_2m_min'][index])),
              isCurved: false,
  ...
  Widget _getTodayView() {
        return LayoutBuilder(
          builder: (context, constraints) {
            double widgetWidth = constraints.maxWidth - 10;
            double widgetHeight = constraints.maxHeight;
            double scale = widgetHeight > widgetWidth ? 0.75 : 0.4;
          return Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Text(TabViewManager.city),
